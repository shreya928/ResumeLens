...
const SmartAnalyzeForm = () => {
  ...
  const feedbackScore = aiFeedback?.score || (aiFeedback?.overallScore ? {
    overall: aiFeedback.overallScore,
    clarity: aiFeedback.clarity || 0,
    relevance: aiFeedback.relevance || 0,
    impact: aiFeedback.impact || 0,
  } : null);
  const feedbackSummary = aiFeedback?.summary || aiFeedback?.overview || "";
  const feedbackPros = aiFeedback?.pros || aiFeedback?.strengths || [];
  const feedbackCons = aiFeedback?.cons || aiFeedback?.weaknesses || [];
  const feedbackSuggestions = aiFeedback?.suggestions || aiFeedback?.recommendations || [];
  const feedbackDetails = aiFeedback?.details || aiFeedback?.analysis || [];
      ...
      {aiFeedback && (
        <div className="mt-8 pt-8 border-t border-slate-200">
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-center">
                        <div className="bg-slate-50 p-4 rounded-lg">
                            <div className="text-3xl font-bold text-blue-600">{feedbackScore.overall}/10</div>
                            <div className="text-sm font-medium text-slate-500 mt-1">Overall</div>
                        </div>
                        {Object.entries(feedbackScore).filter(([key]) => key !== 'overall').map(([key, value]) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                    {feedbackSummary && <div className="mb-6">
                        <h3 className="text-lg font-semibold text-slate-700 mb-2">Summary</h3>
                        <p className="text-slate-600">{feedbackSummary}</p>
                    </div>}
                    
                    {feedbackPros.length > 0 && <div>
                        <h3 className="text-lg font-semibold text-slate-700 mb-2">What's Working Well (Pros)</h3>
                        <ul className="space-y-2">
                            {feedbackPros.map((item, idx) => <li key={idx} className="flex"><CheckIcon /> <span className="text-slate-600">{item}</span></li>)}
                        </ul>
                    </div>}
                </div>

                <div>
                    {feedbackCons.length > 0 && <div className="mb-6">
                        <h3 className="text-lg font-semibold text-slate-700 mb-2">Areas for Improvement (Cons)</h3>
                        <ul className="space-y-2">
                            {feedbackCons.map((item, idx) => <li key={idx} className="flex"><CrossIcon /> <span className="text-slate-600">{item}</span></li>)}
                        </ul>
                    </div>}
                    ...
                    {feedbackDetails.length > 0 && (
                        <div className="mt-8">
                            <h3 className="text-lg font-semibold text-slate-700 mb-3">Detailed Observations</h3>
                            <ul className="space-y-2">
